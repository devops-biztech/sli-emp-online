# Biztech — Online Employment Application (Demo)

A self-contained demo of the online employment application, branded for
Biztech. It looks and behaves like the production app, with three differences:

- **Nothing blocks.** Next always advances, so you can click through an empty
  form. Fields still show their validation messages when you leave them, so
  the checks are visible to anyone who fills the form in.
- **Nothing is submitted.** *Submit application* spins for about a second and
  then shows the real confirmation screen with a made-up confirmation number.
  No encryption, no API route, no AWS. What you type never leaves the tab.
- **It runs offline.** No `.env`, no network. The font ships in the `geist`
  package instead of coming from Google Fonts.

A blue strip on every screen says it is a demo, so nobody mistakes the
confirmation screen for a real application. *Start the demo over* on the
confirmation screen clears the form.

## Run it

```bash
npm install            # needs internet once; everything after is offline
npm run build
npm start              # http://localhost:3000
```

`npm run dev` works too. `?position=Entry%20Level%20Full-time%20Floater`
preselects the position, as in production.

## What changed from production

The demo lives on the `biztech-demo` branch; `main` is the production
Schmidbauer Lumber app.

| Area | Change |
|---|---|
| `src/components/wizard/application-wizard.tsx` | Next doesn't validate; submit is a timed fake; demo strip; restart button; Biztech logo and copy |
| `src/app/page.tsx` | No `SLI_PUB` read or key check |
| `src/app/api/`, `src/lib/encryption.ts`, `src/lib/flatten.ts` | Deleted |
| `package.json` | Dropped `@aws-sdk/client-dynamodb` and `tweetnacl`; added `geist` |
| `src/app/globals.css` | Biztech tokens: green `#1A4321` for structure, blue `#164679` for the primary action |
| `public/` | Biztech logo (reversed) and icon, cropped from `assets/biztech*.png` |

The other documents (`FIELD-INVENTORY.md`, `PRODUCT.md`, `DESIGN.md`) still
describe the production SLI app. The sections below are the production README,
kept for reference.
